...
(() => {
  ...
  async function get() {
    try {
      return fetch("/get").then((res) => res.json().then((res2) => {
        return res2;
      }));
    } catch (err) {
      console.error(err);
  ...
  var overlays = [new Image(), new Image(), new Image(), new Image(), new Image(), new Image(), new Image(), new Image(), new Image(), new Image(), new Image(), new Image(), new Image(), new Image()];
  ...
  function numberChosen(number) {
    ...
    wheel.spinToItem(index, 7e3, false, 2, 1, (x) => -Math.pow(Math.min(1, Math.max(0, -x + 1)), 3) + 1);
    setTimeout(function() {
      if (!wheel) return;
  ...
  var images = [new Image(), new Image(), new Image(), new Image(), new Image(), new Image(), new Image(), new Image(), new Image()];
  ...
  $("#msg").on("click", function() {
    let message = $("#msgText").val()?.toString();
    if (!message) return;
    if (me.spectator || !me.joinedGame) {
      post({ action: "chat", message });
    } else {
      post({ name: me.name, action: "chatInGame", message });
    }
    $("#msgText").val("");
  });
  ...
  var publicVapidKey = "BEORef-fuEOyljiEmeRuLSf17uqmGGKNN0Y4kNF3XbGYr6KfukGSbCj5AkSGsBpT8vUB6GV0cLoZsv9g3MG_XSg";
  ...
  function createEventsFriendMenuButtons() {
    ...
    $(".dm-friend").on("click", function() {
      ...
      dmInterval = setInterval(() => {
        const friend2 = $(this).parent().parent().data("username");
        if (!friend2) return;
        openDmFriendPanel(friend2, true);
        markDmsRead(friend2);
      }, 1e3);
    });
    ...
    $(".invite-friend").on("click", function() {
      ...
      if (!friend) return;
      $("#friendsDiv").attr("inert", "");
      post({ action: "inviteFriend", username: friend }).then((res) => {
        openFriendsPanel();
      });
    });
  ...
  function markDmsRead(friend) {
    ...
    if (unreadMessageReverseIndices.length > 0) {
      post({ action: "readMessages", friend, messageReverseIndices: unreadMessageReverseIndices });
    }
  }
  function openFriendsPanel() {
    ...
    for (let i = 0; i < data.me.friends.length; i++) {
      ...
      const numberOfUnread = dmChannel?.messages.filter((m) => m.from == data.me.friends[i] && !m.read).length;
      $("#friendsDiv").append(`
  ...
  $("#dmMsg").on("click", function() {
    const message = $("#dmMsgText").val()?.toString();
    if (!message) return;
    post({
  ...
  function openDmFriendPanel(friend, reload = false) {
    const messages = data.me.directMessageChannels.find((a) => a.receiver == friend || a.initiatedBy == friend)?.messages;
...
${message.from}: ${message.content}`;
  ...
  setInterval(() => {
    const unreadMessages = data.me.directMessageChannels.find((dm) => {
      return dm.messages.find((m) => m.from != data.me.username && !m.read) != void 0;
    }) != void 0;
    if (unreadMessages) {
